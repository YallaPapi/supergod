...
async def test_process_task_rejects_circular_dependencies(app_with_db):
    app, db, scheduler = app_with_db
    import supergod.orchestrator.server as srv
    from supergod.orchestrator.brain import Subtask
    ...
    await db.create_task("t1", "Build auth system")
    cyclic_subtasks = [
        Subtask(id="1", description="Task 1", depends_on=["2"]),
        Subtask(id="2", description="Task 2", depends_on=["1"]),
    ]
    ...
    task = await db.get_task("t1")
    assert task["status"] == TaskStatus.FAILED
    assert await db.get_subtasks_for_task("t1") == []


async def test_process_task_rejects_unknown_dependencies(app_with_db):
    app, db, scheduler = app_with_db
    import supergod.orchestrator.server as srv
    from supergod.orchestrator.brain import Subtask
    ...
    await db.create_task("t1", "Build auth system")
    invalid_subtasks = [
        Subtask(id="1", description="Task 1", depends_on=[]),
        Subtask(id="2", description="Task 2", depends_on=["missing"]),
    ]

    with patch(
        "supergod.orchestrator.server.decompose_task",
        return_value=invalid_subtasks,
    ):
        await srv._process_task("t1", "Build auth system")
