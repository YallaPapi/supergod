...
**Dependency graph guardrails** — During decomposition, subtask graphs are validated for duplicate IDs, missing dependencies, self-dependencies, and cycles. Invalid plans automatically fall back to a single safe subtask instead of deadlocking the task lane.
