...
async def backtest_ngram_rules(batch_size: int = 500) -> dict:
    ...
    or tier), then evaluates each rule against all markets. Stores results
    in backtest_results and rule_category_performance tables.
    ...
    for batch_start in range(0, len(parsed_rules), batch_size):
        ...
        for rule in batch:
            ...
            if not trades:
                # Still store a zero-match result so we know this rule was tested
                bt = BacktestResult(
                    rule_id=rule["id"],
                    total_matches=0,
        ...
        async with SessionLocal() as session:
            for rule_id, bt, cat_rows in batch_results:
                # Delete old results for this rule
...
async def _main():
    ...
    if args.rule_type in ("ngram", "all"):
        results["ngram"] = await backtest_ngram_rules(batch_size=args.batch_size)
